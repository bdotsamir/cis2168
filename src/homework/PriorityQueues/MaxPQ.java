...
public class MaxPQ<Key extends Comparable<Key>> implements Iterable<Key> {
  ...
  private void sink(int k) {
    while (2 * k <= n) {
      int child = 2 * k;
      if (child < n && less(child, child + 1)) child++; // actually right child
  ...
  /**
   * Compares elements i & j. True if element at i is less than element at j, false if equal to or greater than.
   * @param i Array index
   * @param j Array index
   * @return True if element at i is less than element at j.
   */
